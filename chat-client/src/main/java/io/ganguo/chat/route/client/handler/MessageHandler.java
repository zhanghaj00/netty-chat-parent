...
import io.ganguo.chat.client.MessageObservable;
...
public class MessageHandler extends IMHandler<IMRequest> {
    ...
    private void receiveMessage(IMConnection connection, IMRequest request) {
        ...
        MessageObservable.getInstance().setMessage(message);
        ...
        resp.writeEntity(new AckDTO(message.getFrom(), message.getId(),message.getFromServer()));
